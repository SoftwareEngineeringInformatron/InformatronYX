					...
					if($(this).val().length>=minChars){
						$.ajax({
							...
							success:function(data){
                                                                //console.log( data.site_user_autosuggest );
                                                                //var x = $.parseJSON(data);
                                                                
                                                               
                                                                //console.log(x.site_user_autosuggest);
								try{
                                                                    if( textField.context.alt == 'student' ){
									me.arrData=data.site_user_autosuggest;
                                                                        var arr=data.site_user_autosuggest;
                                                                    }else{
                                                                        me.arrData=data.lo_autosuggest;
                                                                        var arr=data.lo_autosuggest;
                                                                    }
											...
											for(i=0;i<arr.length;i++){
												...
												if(me.thumbnail==true){
													...
													if(arr[i].thumbnail!=undefined){
														style=' style="background-image:url(' + arr[i].thumbnail + '); background-size: 80px 80px;"';
													}
									...
									alert('Sorry, an error has occured! (try)');
									console.log(data);
								}
							},
							error: function(xhr, status, ex){
								alert('Sorry, an error has occured! (success)');
								alert(xhr);
								alert(status);
								alert(ex);
							}
